...
namespace CHS.Web.Areas.Students.Pages.Home
{
    ...
    public partial class SearchTrainer : ComponentBase
    {
        ...
        {
            var response = await Service.SendRequest("Get", $"SearchTrainer");
        ...
        private async Task ChangeFilter(string e)
        {
            ...
            {
                _filter = e;
                var response = await Service.SendRequest("Get", $"SearchTrainer/GetByDesignation?searchTerm={_filter}");
                if (response != null)
                {
                    _searchViewModels = await Service.GetAll<SearchViewModel>(response);
                }
        ...
        private async Task ChangeSearchTerm(string e)
        {
            ...
            if (!string.IsNullOrEmpty(_searchBy) && !string.IsNullOrEmpty(e))
            {
                ...
                {
                    var response = await Service.SendRequest("Get", $"SearchTrainer/GetByName?searchTerm={e}");
                    if (response != null)
                    {
                        _searchViewModels = await Service.GetAll<SearchViewModel>(response);
                    }
                ...
                {
                    var response = await Service.SendRequest("Get", $"SearchTrainer/GetByDesignation?searchTerm={e}");
                    if (response != null)
                    {
                        _searchViewModels = await Service.GetAll<SearchViewModel>(response);
                    }
                ...
                {
                    var response = await Service.SendRequest("Get", $"SearchTrainer/GetByDistrict?searchTerm={e}");
                    if (response != null)
                    {
                        _searchViewModels = await Service.GetAll<SearchViewModel>(response);
                    }
                ...
                {
                    var response = await Service.SendRequest("Get", $"SearchTrainer/GetByCity?searchTerm={e}");
                    if (response != null)
                    {
                        _searchViewModels = await Service.GetAll<SearchViewModel>(response);
                    }
        ...
        private async Task GroupTrainerFilter(bool searchTerm)
        {
            if (searchTerm)
            {
                var response = await Service.SendRequest("Get", $"SearchTrainer/GetByGroupTrainer?searchTerm={searchTerm}");
                if (response.IsSuccessStatusCode)
                {
                    _searchViewModels = await Service.GetAll<SearchViewModel>(response);
                }
        ...
        private async Task PrivateTrainerFilter(bool searchTerm)
        {
            if (searchTerm)
            {
                var response = await Service.SendRequest("Get", $"SearchTrainer/GetByPrivateTrainer?searchTerm={searchTerm}");
                if (response.IsSuccessStatusCode)
                {
                    _searchViewModels = await Service.GetAll<SearchViewModel>(response);
                }
